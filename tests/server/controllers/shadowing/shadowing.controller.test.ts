...
import { createShadowingController } from "../../../../server/src/controllers/shadowing/shadowing.controller";
...
/**
 * Constructs the shadowing controller using three mock repositories.
 *
 * @returns The controller and the mock repos.
 */
const createController = (deps: { transcribeWithOpenAI?: (file: any) => Promise<string> } = {}) => {
  ...
  const controller = createShadowingController(dataSource, deps);
...
describe("Shadowing controller", () => {
  ...
  it("creates a new card when reviewing a new message", async () => {
    ...
    await controller.reviewShadowing(
      authRequest({ body: { rating: 3, messageId: "msg-1", userTranscript: "Xin chao" } }),
      response
    );
  ...
  it("transcribes audio payload", async () => {
    const transcribeWithOpenAI = vi.fn().mockResolvedValue("Xin chao");
    const { controller } = createController({ transcribeWithOpenAI });
    const response = createMockResponse();
    ...
    expect(response.json).toHaveBeenCalledWith({ transcript: "Xin chao" });
  });
});
